 ...
 * @author user
 */
@Named(value = "mpCountryMb")
@ViewScoped
public class MpCountryMB implements Serializable {
    ...
    public void chooseItem() {
        ...
        options.put("dir", "rtl");
        PrimeFaces.current().dialog().openDynamic("newCountry", options, null);
    }

    public void onItemChosen(SelectEvent event) {
        MpCountry newItem = (MpCountry) event.getObject();
        FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "تم اضافة مدينة جديدة",newItem.getName());
    ...
    private MpCountry item;
    private List<MpCountry> items;

    @Inject
    private MpCountryBean bean;
//    @Inject
...
//        prepareCreate();
    ...
    public MpCountry getItem() {
        return item;
    }

    public void setItem(MpCountry item) {
        this.item = item;
    }

    public List<MpCountry> getItems() {
        return items;
    }

    public void setItems(List<MpCountry> items) {
        this.items = items;
    }
